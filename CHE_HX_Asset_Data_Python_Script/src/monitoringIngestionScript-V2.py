        ...
        def asset_data_info_insert(self, path, dest, status, count_Of_Data):
            try:
             ...
             if(os.path.exists(path)):
                ...
                object_to_insert['start_date'] = str(datetime.datetime.strptime(str(df_start_end_date[1][0]), '%Y-%m-%d %H:%M:%S'))
                object_to_insert['end_date'] = str(datetime.datetime.strptime(str(df_start_end_date[1][1]), '%Y-%m-%d %H:%M:%S'))
