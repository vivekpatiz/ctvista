...
import { ViewEditCondenserComponent } from './view-edit-condenser.component';
...
import { ViewCondenserStepperComponent } from './view-condenser-stepper/view-condenser-stepper.component';
...
describe('ViewEditCondenserComponent', () => {
  let component: ViewEditCondenserComponent;
  let service: ViewEditCondenserService;
  let fixture: ComponentFixture<ViewEditCondenserComponent>;
  ...
  const viewEditCondenserServiceStub = () => ({
    getJSON: () => of({
  ...
  beforeEach(async(() => {
    TestBed.configureTestingModule({
      ...
      providers: [
        ...
        {provide: ViewEditCondenserService, useFactory: viewEditCondenserServiceStub},
        {provide: Router, useFactory: routerStub}, 
        {provide: ActivatedRoute, useFactory: activatedRouteStub}
      ]
    });
    fixture = TestBed.createComponent(ViewEditCondenserComponent);
    // TestBed.inject(ViewEditCondenserService);
  ...
  beforeEach(() => {
    ...
    component.setFormValue(data.Condenser,data.unitData,data.unitData,data.historianMap);
    component.getInitValue('condenser_cw_in_temps');
    component.getInitValue('condenser_cw_out_temps');
    component.getInitValue('boiler_steam_flow');
    component.getInitValueNonMand('steam_turbine_output');
    component.allUom = umo;
    parent['ctvpapp '] = {user : {id: 1}};
    parent['get_storage'] = () => ({account: {name: 'Demo Company', id: 1207 }, facility: {name: 'Demo Facility', id: 1208 } })
    component.accountId = 1207;
    component.facilityName = 'Demo Facility';
    component.recentUserAccId = 1207;
    component.fileName = 'xyz';
  });

  it('should create', () => {
    expect(component).toBeTruthy();
  });
  it('should call edit method', fakeAsync(() => {
    component.onEdit('e');
    component.enblEditBtn = false;
    component.enblSaveBtn = true;
    component.formGroup.enable();
  }));
  it('should call save method', fakeAsync(() => {
    const toastSuccessMesage = {
      severity: 'success',
      summary: 'success',
    };
    component.toastService.showSuccess(toastSuccessMesage);
    component.formGroup.disable();
    component.enblSaveBtn = false;
    component.enblEditBtn = true;
    component.selectedFacility = "Demo Facility"
    component.onSave('');
  }));
 
  it('should set value in form on view mode', () => {
    const cndsrname = 'PB12344';
    // component.setFormValue(cndsrname, {}, {}, {});
    component.formGroup.get('condenserName').patchValue(cndsrname);
  });
  it('call getter', () => {
    spyOnProperty(component, 'f').and.callThrough();
  ...
  it('should getFieldAndLabel to generate attemperation Flow field',()=>{
    ...
    component.getInitValueNonMand('attemperation_flow');
    component.getFieldAndLabel(eve);
    spyOn(component,'getFieldAndLabel');
    //  fixture.detectChanges();
    fixture.whenStable().then(() => {
  ...
  it('should getFieldAndLabel to generate Steam Turbine Output field',()=>{
    ...
    //  fixture.detectChanges();
    fixture.whenStable().then(() => {
      expect(component.genDynamicNonMandtryMultplFld).toHaveBeenCalledWith(eve[0],'256','steam_turbine_output');
    });
  });
  it('should getFieldAndLabel to generate attemperation Flow field',()=>{
    const eve = [2,'attemperation Flow'];
    ...
    //  fixture.detectChanges();
    fixture.whenStable().then(() => {
      expect(component.genDynamicNonMandtryMultplFld).toHaveBeenCalledWith(eve[0], 'MW', 'attemperation_flow');
    });
  });
  ...
  //     'uomName': new FormControl('409')
  //   });
  //   const eve = ['409','Air Removal'];
  //   (component.formGroup.get('historianMap').get('air_removal') as FormArray).push(control)
  //   component.patchToSimilar(eve[0],'air_removal');
  //   spyOn(component,'getFieldAndLabel');
  //   // component.getUnitNameById('409');
  //    spyOn(component,'getUnitNameById');
  //     // fixture.detectChanges();
  //   fixture.whenStable().then(() => {
  //     expect(component.formGroup.get('historianMap').get('air_removal')).toBeTruthy();
  //   });
  // });
  it('should be able to set form data', () => {
    component.facilityName =  'b';
    component.setFormValue(
      ...
      },
      {
        apple: {
          key: [
            "gross_load",
            "condenser_back_pressure",
            "boiler_steam_flow",
            "attemperation_flow",
            "cogen_steam_flow",
            "condenser_cw_in_temps",
            "condenser_cw_out_temps",
           "net_load",
           "gross_heat_rate",
           "net_heat_rate",
           "ambient_temp",
  ...
  it('should call onEdit', () => {
    component.onEdit('e');
    expect(component.enblEditBtn).toBe(false);
    expect(component.enblSaveBtn).toBe(true);
    expect(component.isDisabled).toBe(true);
    expect(component.formGroup.enable());
  });
  ...
  it('should call saveTemplate', () => {
    ...
    expect(component.fileName ).toBe('Condenser_3112');
    expect(component.excelName).toBe('Condenser_3112');
  });
  ...
  it('should call clearExcel', () => {
    ...
    expect(component.enblEditBtn ).toBe(true);
    expect(component.enblSaveBtn ).toBe(false);
  });

  it('should call downloadTemplate menthod', fakeAsync(() => {
    spyOn(component, 'downloadTemplate');
    const button = fixture.debugElement.nativeElement.querySelector('button');
    button.click();
    fixture.whenStable().then(() => {
      expect(component.downloadTemplate).toHaveBeenCalled();
    });
  }));
  ...
  it('should call tabChanged', () => {
    const eve = [2,'Air Removal'];
    component.tabChanged(eve);
  });

  it('should call onFacilitySelected', () => {
    component.userFacility = facilitydd;
    const eve = {value: "Demo Facility"};
    component.onFacilitySelected(eve,false);
    expect(component.disableSystem).toBe(false);
  });

  it('should be able to download excel', () => {
